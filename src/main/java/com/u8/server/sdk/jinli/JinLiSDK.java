...
public class JinLiSDK implements ISDKScript{
    ...
    public void verify(final UChannel channel, String extension, final ISDKVerifyListener callback) {
        ...
        try{
            ...
            tokenObject.put("h",jsonObject.get("h"));
            tokenObject.put("t",jsonObject.get("t"));
            tokenObject.put("n",jsonObject.get("n"));
            tokenObject.put("v",jsonObject.get("v"));
            token = tokenObject.toString();
            final String username = json.getString("userId");
    ...
    public void onGetOrderID(UUser user, final UOrder order, final ISDKOrderListener callback) {
        ...
        try{
            ...
            final String out_order_no = order.getOrderID()+"";
            String subject = order.getProductName();
            String submit_time = TimeUtils.format_yyyyMMddHHmmss(new Date());
            String total_fee = deal_price;
            StringBuilder sb = new StringBuilder();
            sb.append(api_key).append(deal_price).append(deliver_type).append(out_order_no).append(player_id).append(subject).append(submit_time).append(total_fee);
            ...
            Map<String,String> headers = new HashMap<String, String>();
            headers.put("Content-Type", "application/json");
            UHttpAgent.getInstance().post(order.getChannel().getChannelOrderUrl(), headers, new ByteArrayEntity(param.getBytes(Charset.forName("UTF-8"))), new UHttpFutureCallback() {
                @Override
                public void completed(String content) {
                    ...
                    try{
                        ...
                        if("200010000".equals(status)){
                            String back_order_no = json.getString("order_no");
                            ...
                            order.setChannelOrderID(back_order_no);
                            orderManager.saveOrder(order);

                            JSONObject json2 = new JSONObject();
                            json2.put("out_order_no", back_order_no);
