 ...
 */
@Controller
@Namespace("/pay/miqiwan")
public class MiQiWanPayCallbackAction extends UActionSupport{
    @Autowired
    ...
    public void payCallback() {
        ...
        if (params == null) {
            Log.e("------米奇玩 pay callBack | message is null");
        ...
        if (order == null || order.getChannel() == null) {
            Log.d("------米奇玩---------The order is null or the channel is null");
            this.renderState(false);
            return;
        }
        if (order.getState() > PayState.STATE_PAYING) {
            Log.d("------米奇玩---------The state of the order is complete  |  The state is " + order.getState());
