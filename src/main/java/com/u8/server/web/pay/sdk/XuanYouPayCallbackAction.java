...
@Namespace("/pay/xuanyou")
public class XuanYouPayCallbackAction extends UActionSupport {
    ...
    public void payCallback() {
        logger.info("----轩游支付回调参数out_trade_no:{}",out_trade_no);
        logger.info("----轩游支付回调参数price:{}",price);
        logger.info("----轩游支付回调参数pay_status:{}",pay_status);
        logger.info("----轩游支付回调参数extend:{}",extend);
        logger.info("----轩游支付回调参数signType:{}",signType);
        logger.info("----轩游支付回调参数sign:{}",sign);
        if (StringUtils.isEmpty(sign) || StringUtils.isEmpty(extend)) {
            logger.warn("----轩游支付回调参数sign、extend错误");
            renderText("fail");
            return;
        }
        ...
        logger.debug("----轩游支付回调key:{}",key);
        if (null == order || null == channel) {
            logger.warn("----轩游支付回调查询渠道和订单为null,orderID:{},channelID:{}",orderID,order.getChannelID());
            renderText("fail");
            return;
        }
        if (SDKStateCode.LOGINSUCCESS != pay_status) {
            logger.warn("----轩游支付回调参数pay_status错误,pay_status:{}",pay_status);
            renderText("fail");
            return;
        }
        StringBuilder sb = new StringBuilder();
        sb.append(out_trade_no).append(String.format("%.2f",price)).append(pay_status).append(extend).append(key);
        logger.debug("----轩游支付回调签名体:{}",sb.toString());
        String checkSign = EncryptUtils.md5(sb.toString()).toLowerCase();
        if (!sign.equals(checkSign)) {
            logger.warn("----轩游支付回调验签失败:{}",sign);
            renderText("fail");
            return;
        }
        synchronized (this) {
            if (order.getState()>PayState.STATE_PAYING) {
                logger.warn("-----轩游支付回调查询订单状态u8已经处理完成,state:{}",order.getState());
                renderText("success");
                return;
            }
            //u8处理订单
