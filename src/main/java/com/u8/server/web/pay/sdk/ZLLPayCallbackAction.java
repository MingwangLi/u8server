...
@Controller
@Namespace("/pay/zll")
public class ZLLPayCallbackAction extends UActionSupport {
    @Autowired
    ...
    public void payCallback() throws IOException {
        ...
        if (order == null || order.getChannel() == null) {
            Log.d("The ZLL order is null");
            this.renderState(false);
            return;
        }
        if (order.getState() > PayState.STATE_PAYING) {
            Log.d("The ZLL Pay state error");
            this.renderState(true);
            return;
        }
        if (!isSignOK(params,order.getChannel())){
            Log.d("The ZLL sign is error");
            this.renderState(false);
            return;
        }
        if(Integer.valueOf(params.get("pay_status")) == 1){
