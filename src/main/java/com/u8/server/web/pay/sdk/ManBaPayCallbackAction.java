...
@Controller
@Namespace("/pay/manba")
public class ManBaPayCallbackAction extends UActionSupport{
    @Autowired
    ...
    public void payCallback() {
        ...
        if (params == null) {
            Log.e("------曼巴 pay callBack | message is null");
            this.renderState(false);
            return;
        }
        long orderID = Long.parseLong(params.get("out_order_no"));
        UOrder order = orderManager.getOrder(orderID);

        if (order == null || order.getChannel() == null) {
            Log.d("------曼巴---------The order is null or the channel is null");
            this.renderState(false);
            return;
        }
        if (order.getState() > PayState.STATE_PAYING) {
            Log.d("------曼巴---------The state of the order is complete  |  The state is " + order.getState());
        ...
        order.setChannelOrderID(params.get("order_no"));
